...
const JoinUs = () => {
  ...
  useEffect(() => {
    setAllCities(
      allCitiesResults?.winnibook_cities.reduce(
        (obj, item) => [...obj, { value: item.id, name: item.name }],
        []
      )
    );
  }, [allCitiesResults]);
  ...
  const addLocal = async () => {
    ...
    if (
      !newLocalAddress.street_line_1 ||
      newLocalAddress.street_line_1 === '' ||
      newLocalAddress.street_line_1.length < 3
    ) {
    ...
    let variables = {
      ...
      categories_ids: newLocalCategories
        .filter(item => item.parent_category_id && item.parent_category_id !== '')
        .reduce((obj, item) => [...obj, { categories_id: item.id }], []),
      tags_ids: newLocalTags.reduce((obj, item) => [...obj, { tags_id: item.id }], []),
      links: newLocalLinks.reduce(
        (obj, item) => [...obj, { link: { data: { name: item.name, url: item.url } } }],
        []
      ),
      contacts: newLocalContacts.reduce(
        (obj, item) => [
          ...obj,
          {
            contact: {
    ...
    if (
      variables.name &&
      variables.name !== '' &&
      variables.main_category_id &&
      variables.main_category_id !== '' &&
      variables.street_line_1 &&
      variables.street_line_1 !== '' &&
      variables.postcode &&
      variables.postcode !== '' &&
      variables.city_id &&
          ...
          <>
            <div className="w-full flex p-4 justify-between">
              <div>
                ...
                <p>
                  - Fill the form below with your business information.
                  <br />- Once we confirm your information your business will be live!
                </p>
              </div>
            </div>
            ...
            <div className="flex-grow flex flex-wrap py-4 border-b">
              ...
              */}
              <div className="px-4 mb-4 w-full">
                <h4 className="mb-2 px-4">Name</h4>
                <Input
                ...
                {errors.name && (
                  <span className="text-red-500">
                    Name is mandatory and must be longer than 3 characters
                  </span>
                )}
              </div>
              ...
              */}
              <div className="px-4 mb-4 w-full">
                <h4 className="mb-2 px-4">Short Description</h4>
                <div
                  className="
                    w-full 
                    h-24 
              ...
              <div className="md:py-8 flex-grow flex flex-wrap">
                ...
                */}
                <div className="px-4 md:w-1/2">
                  <h4 className="mb-2 px-4">Categories</h4>
                  ...
                  {!showCategoriesSelector && newLocalCategories?.length > 0 && (
                    <div className="w-full flex flex-wrap mb-4">
                      {newLocalCategories.map((item, index) => (
                        <div
                          onClick={() =>
                            setNewLocalCategories(
                              newLocalCategories.filter(cat => cat.id !== item.id)
                            )
                          }
                        >
                          <Tag name={item?.name} tagInfo={item} theme={item?.theme} big />
                        </div>
                      ))}

                      <div onClick={() => setShowCategoriesSelector(true)}>
                        <Tag name="Add more" filterTag big />
                      </div>
                    </div>
                  )}
                ...
                */}
                <div className="px-4 md:w-1/2">
                  <h4 className="mb-2 px-4">Tags</h4>
                  ...
                  {!showTagsSelector && newLocalTags?.length > 0 && (
                    <div className="w-full flex flex-wrap mb-4">
                      {newLocalTags.map((item, index) => (
                        <div
                          onClick={() =>
                            setNewLocalTags(newLocalTags.filter(cat => cat.id !== item.id))
                          }
                        >
                          <Tag name={item?.name} tagInfo={item} filterTag big />
                        </div>
                      ))}

                      <div onClick={() => setShowTagsSelector(true)}>
                        <Tag name="Add more" filterTag big />
                      </div>
                    </div>
                  )}
                  ...
                  {(showTagsSelector || newLocalTags?.length <= 0) && (
                    <TagsFinder onSelectTag={selectTag} />
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="flex-grow flex flex-wrap py-4 border-b">
              ...
              <div className="md:px-4 flex-grow flex flex-wrap">
                <div className="px-4 w-full md:w-1/2">
                  <h4 className="mb-2 md:px-4">Links</h4>
                  <LinksEditor onLinksChange={value => setNewLocalLinks(value)} />
                </div>

                <div className="px-4 w-full md:w-1/2">
                  <h4 className="mb-2 md:px-4">Contacts</h4>
                  <ContactsEditor
            ...
            <div className="w-full flex flex-wrap justify-end px-4 py-8">
              <div className="w-full md:w-1/3 mb-8 flex flex-wrap justify-end">
                <p className="w-full mb-4">
                  Please review our{' '}
                  <a className="underline" href="/terms-of-use" target="_blank">
                    Terms of Use
                  </a>{' '}
                  before proceeding.
                </p>
              ...
              <div className="w-full flex justify-end actions">
                <Button title="Submit your place" onClick={() => addLocal()} />
              </div>
            </div>
          </>
        ) : (
              ...
              <div>
                ...
                <p className="mb-4">
                  <span className="text-xl">
                    Your business if under review and will be live soon!
                  </span>
                </p>
              </div>
            </div>
